...
Source: https://github.com/DeepPSP/torch_ecg/blob/master/torch_ecg/databases/physionet_databases/cinc2021.py
"""
...
if cfg_cinc2021.ModelCfg.torch_dtype.lower() == "double":
...
class CINC2021Dataset(Dataset):
    ...
    def __init__(
        self, config: Optional[cfg_cinc2021.CFG] = None, training: bool = True, lazy: bool = True
    ) -> NoReturn:
        """
        Parameters
        ----------
        ...
        self._indices = [cfg_cinc2021.Standard12Leads.index(ld) for ld in self.config.leads]
        ...
        ppm_config = cfg_cinc2021.CFG(random=False)
    ...
    def _load_one_record(self, rec: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for idx in range(values.shape[0]):
            values[idx] = utils_cinc2021.remove_spikes_naive(values[idx])
        values, _ = self.ppm(values, self.config.fs)
        values = utils_cinc2021.ensure_siglen(
            values,
            siglen=self.siglen,
    ...
    def empty(self, leads: Optional[Sequence[str]] = None) -> NoReturn:
        """ """
        if leads is None:
            leads = self.config.leads
        else:
    ...
    @classmethod
    def from_extern(cls, ext_ds, config) -> "CINC2021Dataset":
        """ """
    ...
    def _train_test_split(
        ...
        if self.training == "all":
            records = utils_cinc2021.list_sum([train_set[k] for k in _tranches]) + utils_cinc2021.list_sum(
                [test_set[k] for k in _tranches]
            )
        elif self.training is True:
            records = utils_cinc2021.list_sum([train_set[k] for k in _tranches])
        else:
            records = utils_cinc2021.list_sum([test_set[k] for k in _tranches])
        return records
    ...
    def _check_train_test_split_validity(
        ...
        """
        train_classes = set(
            utils_cinc2021.list_sum([self.reader.get_labels(rec, fmt="a") for rec in train_set])
        )
        train_classes.intersection_update(all_classes)
        test_classes = set(
            utils_cinc2021.list_sum([self.reader.get_labels(rec, fmt="a") for rec in test_set])
        )
...
def _load_record(reader: CR, rec: str, config: cfg_cinc2021.CFG) -> Tuple[np.ndarray, np.ndarray]:
    ...
    values = utils_cinc2021.ensure_siglen(values, siglen=config.input_len, fmt="channel_first")
