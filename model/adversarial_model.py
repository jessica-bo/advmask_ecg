...
class AdversarialModel(BaseModel):
    def __init__(
        ...
        **kwargs):
        ...
        # Params 144,246,017
        self.mask_encoder = unet1Dsmall(input_dim=12, embedding_dim=self.encoder.embedding_dim, depth=1, nmasks=self.nmasks)
        # self.mask_encoder = unet1D(input_dim=12, embedding_dim=self.encoder.embedding_dim, depth=1, nmasks=self.nmasks)
    ...
    def log_mask(self, x_orig, mask):
        ...
        if self.nmasks == 1:
            ...
            for i in range(self.nmasks):
                ...
                final_mask_i = final_mask[i].detach().cpu().numpy()
